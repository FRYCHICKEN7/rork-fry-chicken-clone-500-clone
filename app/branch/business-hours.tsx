...
export default function BranchBusinessHoursScreen() {
  const { colors } = useTheme();
  const { user } = useAuth();
  const { branches, businessHours, updateBranchBusinessHours, isBranchOpen, getNextOpenTimeForBranch } = useData();
  
  const branch = branches.find(b => b.id === user?.branchId);
  const currentBranchHours = branch?.businessHours || businessHours;
  
  const [localHours, setLocalHours] = useState<BusinessHours[]>(currentBranchHours);
  ...
  const handleSave = async () => {
    if (!user?.branchId) {
      Alert.alert("Error", "No se encontró la sucursal");
      return;
    }
    
    try {
      console.log('💾 [BRANCH HOURS] Saving hours for branch:', user.branchId);
      console.log('💾 [BRANCH HOURS] New hours:', localHours);
      await updateBranchBusinessHours(user.branchId, localHours);
      setHasChanges(false);
      Alert.alert("Éxito", "Horarios de la sucursal actualizados correctamente");
    } catch (error: any) {
      console.error('❌ [BRANCH HOURS] Error saving hours:', error);
      Alert.alert("Error", error?.message || "No se pudieron guardar los horarios. Revisa los logs de la consola para más detalles.");
    }
  };
  ...
  const applyToAll = (sourceDay: number) => {
    ...
    Alert.alert(
      ...
      `¿Deseas aplicar el horario de ${DAY_NAMES[sourceDay]} a todos los días de esta sucursal?`,
      [
  ...
  const branchOpen = user?.branchId ? isBranchOpen(user.branchId) : false;
  const nextOpen = user?.branchId ? getNextOpenTimeForBranch(user.branchId) : "";

  const styles = createStyles(colors);

  return (
    <SafeAreaView style={styles.container} edges={["bottom"]}>
      <Stack.Screen options={{ title: "Horarios de Sucursal" }} />

      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          <Clock size={24} color={colors.primary} />
          <Text style={styles.title}>Horarios de {branch?.name || "Sucursal"}</Text>
        </View>
        <Text style={styles.subtitle}>
          Configura los horarios de apertura y cierre de tu sucursal
        </Text>

        <View style={[styles.statusCard, { backgroundColor: branchOpen ? colors.success + "20" : colors.accent + "20" }]}>
          <View style={[styles.statusIndicator, { backgroundColor: branchOpen ? colors.success : colors.accent }]} />
          <View style={styles.statusContent}>
            <Text style={[styles.statusTitle, { color: branchOpen ? colors.success : colors.accent }]}>
        ...
        <View style={styles.infoCard}>
          ...
          <Text style={styles.infoText}>
            Estos horarios solo aplican para tu sucursal. Cuando el local esté cerrado, los clientes verán una notificación con el próximo horario de atención.
          </Text>
        </View>
